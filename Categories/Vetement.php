...
if($db_found)  
{
	$result = mysqli_query($db_handle, "SELECT * FROM Vetement ORDER BY ID_Produit" );
	while($data = mysqli_fetch_assoc($result)) 
	{
		...
		while($data2 = mysqli_fetch_assoc($resultBIS)) 
		{
			...
			$Couleur[$j] = $data['Couleur'];
			$Taille[$j] = $data['Couleur'];
			$Modele[$j] = $data['Modele'];
			$Genre[$j] = $data['Genre'];
...
<html>
	...
	<body>
		...
		<div id="corps">
			...
			echo "Couleur: ".$Couleur[$i].'</br>';
			echo "Taille: ".$Taille[$i].'</br>';
			echo "Modele: ".$Modele[$i].'</br>';
